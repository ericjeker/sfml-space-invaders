...
ParticleConstellationManager::ParticleConstellationManager(sf::Texture texture)
...
void ParticleConstellationManager::Initialize(const size_t n)
{
	...
	{
		std::uniform_int_distribution dstWidth(1, static_cast<int>(Configuration::WindowSize.x));
		std::uniform_int_distribution dstHeight(1, static_cast<int>(Configuration::WindowSize.y));
...
void ParticleConstellationManager::Update(const sf::Time& deltaTime)
{
	...
	for (size_t i = 0; i < _particles.GetSize(); ++i)
	{
		...
		ScreenWarp(_particles.positions[i], {_particleTexture.getSize().x * _particles.scales[i],
											 _particleTexture.getSize().y * _particles.scales[i]});
	}
...
void ParticleConstellationManager::ScreenWarp(sf::Vector2f& OutPosition, const sf::Vector2f InSize)
{
	...
	{
		OutPosition.x = Configuration::WindowSize.x + InSize.x;
	}

	if (OutPosition.x - InSize.x > Configuration::WindowSize.x)
	{
	...
	{
		OutPosition.y = Configuration::WindowSize.y + InSize.y;
	}

	if (OutPosition.y - InSize.x > Configuration::WindowSize.y)
	{
