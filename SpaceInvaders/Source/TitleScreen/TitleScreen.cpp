...
void TitleScreen::Activate()
{
	...
	// Create Buttons
	auto playButton = std::make_unique<Button>(font, "Play", 40, static_cast<int>(CommandId::StartGame));
	playButton->SetSize({200, 50});
	playButton->SetPosition({centerX - 100, centerY - 25});
	_uiManager.AddComponent(std::move(playButton));
	auto exitButton = std::make_unique<Button>(font, "Exit", 30, static_cast<int>(CommandId::Quit));
