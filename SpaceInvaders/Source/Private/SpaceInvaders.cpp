    ...
    , _logger("SpaceInvaders", configuration.LogLevel)
{
    ...
    _screenManager.RegisterScreen(typeid(TitleScreen), [this]() { return std::make_unique<TitleScreen>(*this); });
    _screenManager.RegisterScreen(typeid(GameScreen), [this]() { return std::make_unique<GameScreen>(*this); });
